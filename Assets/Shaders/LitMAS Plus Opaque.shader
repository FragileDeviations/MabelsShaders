...
Shader "Mabel/LitMAS Plus/LitMAS+ Opaque"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float MyCustomExpression217_g269( float detailSmooth, float smoothness )
			{
				...
				) : SV_Target
			{
				...
				float staticSwitch324 = 0.0;
				#else
				float staticSwitch324 = _ParallaxScale;
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float staticSwitch330 = _ParallaxScale;
				#else
				float staticSwitch330 = staticSwitch324;
				#endif
				...
				#else
				float staticSwitch323 = _MinSamples;
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float staticSwitch326 = _MinSamples;
				#else
				float staticSwitch326 = staticSwitch323;
				#endif
				#ifdef SHADER_API_MOBILE
				float staticSwitch322 = 0.0;
				#else
				float staticSwitch322 = _MaxSamples;
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float staticSwitch327 = _MaxSamples;
				#else
				float staticSwitch327 = staticSwitch322;
				#endif
				#ifdef SHADER_API_MOBILE
				float staticSwitch321 = 0.0;
				#else
				float staticSwitch321 = _SidewallSteps;
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float staticSwitch328 = _SidewallSteps;
				#else
				float staticSwitch328 = staticSwitch321;
				#endif
				#ifdef SHADER_API_MOBILE
				float staticSwitch325 = 0.0;
				#else
				float staticSwitch325 = _RefPlane;
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float staticSwitch329 = _RefPlane;
				#else
				float staticSwitch329 = staticSwitch325;
				#endif
				float2 OffsetPOM111 = POM( _HeightMap, uvs4_BaseMap.xy, ddx(uvs4_BaseMap.xy), ddy(uvs4_BaseMap.xy), ase_worldNormal, ase_worldViewDir, ase_tanViewDir, (int)staticSwitch326, (int)staticSwitch327, (int)staticSwitch328, staticSwitch330, staticSwitch329, _HeightMap_ST.xy, float2(0,0), 0 );
				#ifdef SHADER_API_MOBILE
				float4 staticSwitch320 = uvs4_BaseMap;
				#else
				float4 staticSwitch320 = float4( OffsetPOM111, 0.0 , 0.0 );
				#endif
				#ifdef _QUESTPARALLAXENABLED_ON
				float4 staticSwitch319 = float4( OffsetPOM111, 0.0 , 0.0 );
				#else
				float4 staticSwitch319 = staticSwitch320;
				#endif
				#ifdef _HEIGHTMAPENABLED_ON
				float4 staticSwitch112 = staticSwitch319;
				#else
				float4 staticSwitch112 = uvs4_BaseMap;
				#endif
				float4 Texture_Coordinates149 = staticSwitch112;
				float2 appendResult230 = (float2(_TriplanarScale , _TriplanarScale));
				float2 TriplanarScale228 = appendResult230;
				...
				float4 staticSwitch225 = ( triplanar226 * _BaseColor );
				#else
				float4 staticSwitch225 = ( tex2D( _BaseMap, Texture_Coordinates149.xy ) * _BaseColor );
				#endif
				float4 temp_output_21_0_g269 = staticSwitch225;
				float4 temp_output_109_0_g270 = round( temp_output_21_0_g269 );
				...
				#endif
				#ifdef _TRIPLANARENABLED_ON
				float4 staticSwitch261 = float4( texCoord203, 0.0 , 0.0 );
				#else
				float4 staticSwitch261 = staticSwitch204;
				#endif
				float2 temp_output_218_0_g269 = staticSwitch261.xy;
				float4 tex2DNode1_g269 = tex2D( _DetailMap, ( ( temp_output_218_0_g269 * _DetailMap_ST.xy ) + _DetailMap_ST.zw ) );
				#ifdef _DETAILS_ON
				float4 staticSwitch16_g269 = ( ( temp_output_109_0_g270 * ( 1.0 - ( ( 1.0 - temp_output_21_0_g269 ) * ( 1.0 - (tex2DNode1_g269.r).xxxx ) * 2.0 ) ) ) + ( ( 1.0 - temp_output_109_0_g270 ) * ( temp_output_21_0_g269 * (tex2DNode1_g269.r).xxxx * 2.0 ) ) );
				#else
				float4 staticSwitch16_g269 = temp_output_21_0_g269;
				#endif
				float4 Albedo138 = staticSwitch16_g269;
				
				float4 tex2DNode8 = tex2D( _BumpMap, Texture_Coordinates149.xy );
				float4 In02_g267 = tex2DNode8;
				float3 localMyCustomExpression2_g267 = MyCustomExpression( In02_g267 );
				float3 temp_output_160_0 = localMyCustomExpression2_g267;
				...
				float4 In02_g268 = triplanar252;
				float3 localMyCustomExpression2_g268 = MyCustomExpression( In02_g268 );
				float3 temp_output_242_0 = localMyCustomExpression2_g268;
				float3 lerpResult245 = lerp( temp_output_242_0 , float3( 0,0,1 ) , step( 0.0 , 0.0 ));
				#ifdef _NORMALS_ON
				float3 staticSwitch247 = temp_output_242_0;
				#else
				float3 staticSwitch247 = lerpResult245;
				#endif
				#ifdef _TRIPLANARENABLED_ON
				float3 staticSwitch258 = staticSwitch247;
				#else
				float3 staticSwitch258 = staticSwitch99;
				#endif
				float4 temp_output_22_0_g269 = float4( staticSwitch258 , 0.0 );
				float2 temp_output_57_0_g269 = (tex2DNode1_g269).ga;
				float2 break79_g269 = temp_output_57_0_g269;
				float3 appendResult56_g269 = (float3(break79_g269.y , break79_g269.x , 1.0));
				float3 temp_cast_23 = (1.0).xxx;
				#ifdef _DETAILS_ON
				float4 staticSwitch26_g269 = float4( BlendNormal( temp_output_22_0_g269.rgb , ( ( appendResult56_g269 * 2.0 ) - temp_cast_23 ) ) , 0.0 );
				#else
				float4 staticSwitch26_g269 = temp_output_22_0_g269;
				#endif
				float4 Normal139 = staticSwitch26_g269;
				
				float4 color104 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
				float dotResult45 = dot( ase_worldViewDir , ase_worldNormal );
				float4 temp_output_29_0 = ( tex2D( _EmissionMap, Texture_Coordinates149.xy ) * _EmissionColor * saturate( pow( abs( dotResult45 ) , _EmissionFalloff ) ) );
				#ifdef _EMISSION_ON
				float4 staticSwitch103 = temp_output_29_0;
				#else
				float4 staticSwitch103 = color104;
				#endif
				float4 color276 = IsGammaSpace() ? float4(0,0,0,0) : float4(0,0,0,0);
				float4 triplanar281 = TriplanarSampling281( _EmissionMap, i.wPos.xyz, ase_worldNormal, 1.0, TriplanarScale228, 1.0, 0 );
				float dotResult268 = dot( ase_worldViewDir , ase_worldNormal );
				float4 temp_output_275_0 = ( triplanar281 * _EmissionColor * saturate( pow( abs( dotResult268 ) , _EmissionFalloff ) ) );
				#ifdef _EMISSION_ON
				float4 staticSwitch278 = temp_output_275_0;
				#else
				float4 staticSwitch278 = color276;
				#endif
				#ifdef _TRIPLANARENABLED_ON
				float4 staticSwitch264 = staticSwitch278;
				#else
				float4 staticSwitch264 = staticSwitch103;
				#endif
				float3 temp_output_13_0_g273 = staticSwitch264.rgb;
				...
				#ifdef _TRIPLANARENABLED_ON
				float staticSwitch255 = triplanar253.y;
				#else
				float staticSwitch255 = tex2DNode9.g;
				#endif
				float Ambient_Occlusion133 = staticSwitch255;
				float localBakerySpecMonoSHFull4_g274 = ( 0.0 );
				float3 mapNormal16_g276 = Normal139.rgb;
				float3 normalizeResult18_g276 = normalize( ( ( ase_worldTangent * mapNormal16_g276.x ) + ( ase_worldBitangent * mapNormal16_g276.y ) + ( ase_worldNormal * mapNormal16_g276.z ) ) );
				float3 normalWorld4_g274 = normalizeResult18_g276;
				float2 lightmapUV4_g274 = (i.ase_texcoord10.xy*(unity_LightmapST).xy + (unity_LightmapST).zw);
				float3 normalizeResult2_g274 = normalize( ase_worldViewDir );
				float3 viewDir4_g274 = normalizeResult2_g274;
				#ifdef _TRIPLANARENABLED_ON
				float staticSwitch256 = saturate( ( ( triplanar252.b + triplanar253.b ) - 1.0 ) );
				#else
				float staticSwitch256 = saturate( ( ( tex2DNode8.b + tex2DNode9.b ) - 1.0 ) );
				#endif
				float temp_output_23_0_g269 = staticSwitch256;
				float detailSmooth217_g269 = tex2DNode1_g269.b;
				float smoothness217_g269 = temp_output_23_0_g269;
				float localMyCustomExpression217_g269 = MyCustomExpression217_g269( detailSmooth217_g269 , smoothness217_g269 );
				#ifdef _DETAILS_ON
				float staticSwitch17_g269 = localMyCustomExpression217_g269;
				#else
				float staticSwitch17_g269 = temp_output_23_0_g269;
				#endif
				float Smoothness134 = staticSwitch17_g269;
				float smoothness4_g274 = Smoothness134;
				float3 temp_output_9_0_g273 = Albedo138.rgb;
				float3 albedo4_g274 = temp_output_9_0_g273;
				#ifdef _TRIPLANARENABLED_ON
				float staticSwitch254 = triplanar253.r;
				#else
				float staticSwitch254 = tex2DNode9.r;
				#endif
				float Metallic132 = staticSwitch254;
				float metalness4_g274 = Metallic132;
				float3 diffuseSH4_g274 = float3( 0,0,0 );
				float3 specularSH4_g274 = float3( 0,0,0 );
				BakerySpecMonoSHFull_float( normalWorld4_g274 , lightmapUV4_g274 , viewDir4_g274 , smoothness4_g274 , albedo4_g274 , metalness4_g274 , diffuseSH4_g274 , specularSH4_g274 );
				#ifdef _MONOSHENABLED_ON
				float3 staticSwitch30_g273 = ( ( temp_output_13_0_g273 + ( Ambient_Occlusion133 * ( diffuseSH4_g274 * temp_output_9_0_g273 ) ) ) + ( specularSH4_g274 * _MonoSHAdjustment ) );
				#else
				float3 staticSwitch30_g273 = temp_output_13_0_g273;
				#endif
				
				#ifdef _TRIPLANARENABLED_ON
				float4 staticSwitch265 = ( temp_output_275_0 * _BakedMutiplier );
				#else
				float4 staticSwitch265 = ( temp_output_29_0 * _BakedMutiplier );
				#endif
			...
			//---------------------------------------------------------------------------------------------------------------------------
				...
				half3 emission = staticSwitch30_g273;
		...
		Pass
		{
				...
				) : SV_Target
			{
			   ...
			   #ifdef _DETAILS_ON
			   float4 staticSwitch26_g269 = float4( BlendNormal( temp_output_22_0_g269.rgb , ( ( appendResult56_g269 * 2.0 ) - temp_cast_17 ) ) , 0.0 );
			   #else
			   float4 staticSwitch26_g269 = temp_output_22_0_g269;
			   #endif
			   float4 Normal139 = staticSwitch26_g269;
		...
		Pass
		{
			...
			half4 frag(v2f i  ) : SV_Target
			{
				...
				#ifdef _DETAILS_ON
				float4 staticSwitch26_g269 = float4( BlendNormal( temp_output_22_0_g269.rgb , ( ( appendResult56_g269 * 2.0 ) - temp_cast_28 ) ) , 0.0 );
				#else
				float4 staticSwitch26_g269 = temp_output_22_0_g269;
				#endif
				float4 Normal139 = staticSwitch26_g269;
				...
				float3 normalizeResult18_g276 = normalize( ( ( ase_worldTangent * mapNormal16_g276.x ) + ( ase_worldBitangent * mapNormal16_g276.y ) + ( ase_worldNormal * mapNormal16_g276.z ) ) );
				float3 normalWorld4_g274 = normalizeResult18_g276;
				float2 lightmapUV4_g274 = (i.ase_texcoord7.xy*(unity_LightmapST).xy + (unity_LightmapST).zw);
				float3 normalizeResult2_g274 = normalize( ase_worldViewDir );
				float3 viewDir4_g274 = normalizeResult2_g274;
				#ifdef _TRIPLANARENABLED_ON
				float staticSwitch256 = saturate( ( ( triplanar252.b + triplanar253.b ) - 1.0 ) );
...
/*ASEBEGIN
Version=19603
Node;AmplifyShaderEditor.CommentaryNode;115;-720,-800;Inherit;False;1676.709;796.0375;;23;149;112;319;320;111;113;106;108;107;109;110;105;321;322;323;324;325;326;327;328;329;330;331;Parallax Mapping;1,1,1,1;0;0
Node;AmplifyShaderEditor.RangedFloatNode;107;-688,-304;Inherit;False;Property;_RefPlane;Ref Plane;31;0;Create;True;0;0;0;False;0;False;0;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;106;-688,-560;Inherit;False;Property;_ParallaxScale;Scale;26;0;Create;False;0;0;0;False;0;False;0.02;0;0;1;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;109;-688,-496;Inherit;False;Property;_MinSamples;Min Samples;28;0;Create;True;0;0;0;False;0;False;8;0;1;128;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;110;-688,-432;Inherit;False;Property;_MaxSamples;Max Samples;29;0;Create;True;0;0;0;False;0;False;16;16;1;128;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;105;-688,-368;Inherit;False;Property;_SidewallSteps;Sidewall Steps;30;0;Create;True;0;0;0;False;0;False;2;2;0;10;0;1;FLOAT;0
Node;AmplifyShaderEditor.RangedFloatNode;331;-656,-208;Inherit;False;Constant;_ZeroConstLol;ZeroConstLol;27;0;Create;True;0;0;0;False;0;False;0;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;321;-352,-432;Inherit;False;Property;_SHADER_API_MOBILE2;SHADER_API_MOBILE;33;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;322;-352,-528;Inherit;False;Property;_SHADER_API_MOBILE3;SHADER_API_MOBILE;33;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;323;-352,-624;Inherit;False;Property;_SHADER_API_MOBILE4;SHADER_API_MOBILE;33;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;324;-352,-720;Inherit;False;Property;_SHADER_API_MOBILE5;SHADER_API_MOBILE;33;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;325;-352,-336;Inherit;False;Property;_SHADER_API_MOBILE1;SHADER_API_MOBILE;33;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.TexturePropertyNode;108;-624,-752;Inherit;True;Property;_HeightMap;Height Map;27;0;Create;True;1;Parallax Occlusion Mapping;0;0;False;0;False;None;None;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TextureCoordinatesNode;113;-336,-208;Inherit;False;0;221;4;3;2;SAMPLER2D;;False;0;FLOAT2;1,1;False;1;FLOAT2;0,0;False;5;FLOAT4;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4
Node;AmplifyShaderEditor.StaticSwitch;330;-80,-720;Inherit;False;Property;_Keyword10;Keyword 10;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;319;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;326;-80,-624;Inherit;False;Property;_Keyword11;Keyword 10;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;319;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;327;-80,-528;Inherit;False;Property;_Keyword12;Keyword 10;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;319;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;328;-80,-432;Inherit;False;Property;_Keyword13;Keyword 10;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;319;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;329;-80,-336;Inherit;False;Property;_Keyword14;Keyword 10;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;319;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ParallaxOcclusionMappingNode;111;224,-720;Inherit;False;0;8;False;;16;False;;2;0.022;0;False;1,1;False;0,0;11;0;FLOAT2;0,0;False;1;SAMPLER2D;;False;7;SAMPLERSTATE;;False;2;FLOAT;0.02;False;3;FLOAT3;0,0,0;False;8;INT;0;False;9;INT;0;False;10;INT;0;False;4;FLOAT;0;False;5;FLOAT2;0,0;False;6;FLOAT;0;False;1;FLOAT2;0
Node;AmplifyShaderEditor.CommentaryNode;236;-2720,-864;Inherit;False;580;187;;3;228;227;230;Triplanar Settings;1,1,1,1;0;0
Node;AmplifyShaderEditor.StaticSwitch;320;592,-720;Inherit;False;Property;_SHADER_API_MOBILE;SHADER_API_MOBILE;30;0;Create;True;0;0;0;False;0;False;0;0;0;False;SHADER_API_MOBILE;Toggle;2;Key0;Key1;Fetch;True;True;All;9;1;FLOAT4;0,0,0,0;False;0;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;3;FLOAT4;0,0,0,0;False;4;FLOAT4;0,0,0,0;False;5;FLOAT4;0,0,0,0;False;6;FLOAT4;0,0,0,0;False;7;FLOAT4;0,0,0,0;False;8;FLOAT4;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.RangedFloatNode;227;-2672,-816;Inherit;False;Property;_TriplanarScale;Scale;23;0;Create;False;0;0;0;False;0;False;1;0;0;0;0;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;319;640,-608;Inherit;False;Property;_QuestParallaxEnabled;Quest Parallax Enabled;25;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;FLOAT4;0,0,0,0;False;0;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;3;FLOAT4;0,0,0,0;False;4;FLOAT4;0,0,0,0;False;5;FLOAT4;0,0,0,0;False;6;FLOAT4;0,0,0,0;False;7;FLOAT4;0,0,0,0;False;8;FLOAT4;0,0,0,0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.RegisterLocalVarNode;149;544,-208;Inherit;False;Texture Coordinates;-1;True;1;0;FLOAT4;0,0,0,0;False;1;FLOAT4;0
...
Node;AmplifyShaderEditor.TexturePropertyNode;237;-3424,160;Inherit;True;Property;_BumpMap;Normal Map;3;2;[NoScaleOffset];[Normal];Create;False;0;0;0;False;0;False;None;None;False;bump;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.TexturePropertyNode;238;-3424,352;Inherit;True;Property;_MetallicGlossMap;MAS;4;1;[NoScaleOffset];Create;False;0;0;0;False;0;False;75f1fbacfa73385419ec8d7700a107ea;75f1fbacfa73385419ec8d7700a107ea;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.CommentaryNode;235;-2576,0;Inherit;False;1038;302;;3;231;226;229;BaseMap Triplanar;1,1,1,1;0;0
Node;AmplifyShaderEditor.CommentaryNode;50;-2576,-560;Inherit;False;987.9999;522.3;;4;5;17;6;206;BaseMap Standard;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.FunctionNode;160;-2000,576;Inherit;False;UnpackNormal;-1;;267;d579cc33c6fa60b4ea9cee9e184b62e3;0;1;1;FLOAT4;0,0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.FunctionNode;242;-1872,1296;Inherit;False;UnpackNormal;-1;;268;d579cc33c6fa60b4ea9cee9e184b62e3;0;1;1;FLOAT4;0,0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.RangedFloatNode;13;32,1184;Inherit;False;Property;_EmissionFalloff;Emission Falloff;8;0;Create;False;0;0;0;True;0;False;1;1;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.StaticSwitch;204;-448,176;Inherit;False;Property;_Keyword0;Keyword 0;24;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;112;True;True;All;9;1;FLOAT4;0,0,0,0;False;0;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;3;FLOAT4;0,0,0,0;False;4;FLOAT4;0,0,0,0;False;5;FLOAT4;0,0,0,0;False;6;FLOAT4;0,0,0,0;False;7;FLOAT4;0,0,0,0;False;8;FLOAT4;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.StaticSwitch;247;-1616,1152;Inherit;False;Property;_Normals1;Normal Map Enabled;2;0;Create;False;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;99;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.GetLocalVarNode;283;-240,1824;Inherit;False;228;TriplanarScale;1;0;OBJECT;;False;1;FLOAT2;0
Node;AmplifyShaderEditor.TexturePropertyNode;263;-896,1280;Inherit;True;Property;_EmissionMap;Emission Map;6;1;[NoScaleOffset];Create;False;0;0;0;False;0;False;None;None;False;white;Auto;Texture2D;-1;0;2;SAMPLER2D;0;SAMPLERSTATE;1
Node;AmplifyShaderEditor.SimpleMultiplyOpNode;231;-1856,80;Inherit;False;2;2;0;FLOAT4;0,0,0,0;False;1;COLOR;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.SaturateNode;48;400,1088;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.ColorNode;12;-176,880;Inherit;False;Property;_EmissionColor;Emission Color;7;1;[HDR];Create;False;0;0;0;True;0;False;1,1,1,1;0,0,0,1;True;True;0;6;COLOR;0;FLOAT;1;FLOAT;2;FLOAT;3;FLOAT;4;FLOAT3;5
Node;AmplifyShaderEditor.SaturateNode;273;384,2240;Inherit;False;1;0;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;261;-336,320;Inherit;False;Property;_Keyword5;Keyword 0;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;FLOAT4;0,0,0,0;False;0;FLOAT4;0,0,0,0;False;2;FLOAT4;0,0,0,0;False;3;FLOAT4;0,0,0,0;False;4;FLOAT4;0,0,0,0;False;5;FLOAT4;0,0,0,0;False;6;FLOAT4;0,0,0,0;False;7;FLOAT4;0,0,0,0;False;8;FLOAT4;0,0,0,0;False;1;FLOAT4;0
Node;AmplifyShaderEditor.StaticSwitch;256;-1040,720;Inherit;False;Property;_Keyword3;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;258;-1120,464;Inherit;False;Property;_Keyword4;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;FLOAT3;0,0,0;False;0;FLOAT3;0,0,0;False;2;FLOAT3;0,0,0;False;3;FLOAT3;0,0,0;False;4;FLOAT3;0,0,0;False;5;FLOAT3;0,0,0;False;6;FLOAT3;0,0,0;False;7;FLOAT3;0,0,0;False;8;FLOAT3;0,0,0;False;1;FLOAT3;0
...
Node;AmplifyShaderEditor.StaticSwitch;225;-1456,-256;Inherit;False;Property;_TriplanarEnabled;Triplanar Enabled;22;0;Create;True;0;0;0;False;3;Space(15);Header(Triplanar);Space(10);False;0;0;0;True;;Toggle;2;Key0;Key1;Create;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
...
Node;AmplifyShaderEditor.StaticSwitch;254;-1184,928;Inherit;False;Property;_Keyword1;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.StaticSwitch;255;-1184,1024;Inherit;False;Property;_Keyword2;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;FLOAT;0;False;0;FLOAT;0;False;2;FLOAT;0;False;3;FLOAT;0;False;4;FLOAT;0;False;5;FLOAT;0;False;6;FLOAT;0;False;7;FLOAT;0;False;8;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;202;-80,160;Inherit;False;Detail Map;10;;269;7eb669e8a4269ac43a3c284ddafaf020;0;4;21;COLOR;1,1,1,1;False;22;COLOR;0,0,1,0;False;23;FLOAT;0.5;False;218;FLOAT2;0,0;False;3;COLOR;0;COLOR;25;FLOAT;24
Node;AmplifyShaderEditor.CommentaryNode;96;1328,-160;Inherit;False;564.4475;280.7469;It's set up this way because otherwise, these properties get ignored.;2;67;317;Surface Properties;1,1,1,1;0;0
...
Node;AmplifyShaderEditor.StaticSwitch;264;1104,752;Inherit;False;Property;_Keyword6;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.RangedFloatNode;67;1488,-64;Inherit;False;Property;_Cull;Cull Side;32;2;[HideInInspector];[Enum];Create;False;0;0;1;UnityEngine.Rendering.CullMode;False;0;False;0;2;0;0;0;1;FLOAT;0
...
Node;AmplifyShaderEditor.StaticSwitch;265;1104,864;Inherit;False;Property;_Keyword7;Keyword 1;22;0;Create;True;0;0;0;False;0;False;0;0;0;True;;Toggle;2;Key0;Key1;Reference;225;True;True;All;9;1;COLOR;0,0,0,0;False;0;COLOR;0,0,0,0;False;2;COLOR;0,0,0,0;False;3;COLOR;0,0,0,0;False;4;COLOR;0,0,0,0;False;5;COLOR;0,0,0,0;False;6;COLOR;0,0,0,0;False;7;COLOR;0,0,0,0;False;8;COLOR;0,0,0,0;False;1;COLOR;0
Node;AmplifyShaderEditor.StepOpNode;317;1664,-64;Inherit;False;2;0;FLOAT;0;False;1;FLOAT;0;False;1;FLOAT;0
Node;AmplifyShaderEditor.FunctionNode;306;1344,304;Inherit;False;BakeryMonoSH;16;;273;29c9468cd28079b448a58bef1fb32cb5;0;6;8;FLOAT3;0,0,0;False;9;FLOAT3;0,0,0;False;10;FLOAT;0;False;11;FLOAT;0;False;12;FLOAT;0;False;13;FLOAT3;0,0,0;False;1;FLOAT3;0
Node;AmplifyShaderEditor.TemplateMultiPassMasterNode;312;2272,208;Float;False;True;-1;2;UnityEditor.ShaderGraphLitGUI;0;14;Mabel/LitMAS Plus/LitMAS+ Opaque;623634af11bd9ab448550ee777f3493e;True;Forward;0;0;Forward;14;False;True;1;1;False;_BlendSrc;0;False;_BlendDst;0;1;False;;0;False;;False;False;False;False;False;False;False;False;False;False;False;True;True;0;True;_Cull;False;True;True;True;True;True;0;False;;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;True;1;False;_ZWrite;True;3;False;;True;True;0;False;;0;False;;True;3;RenderPipeline=UniversalPipeline;RenderType=Opaque=RenderType;Queue=Geometry=Queue=0;False;False;0;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;False;True;False;0;False;;255;False;;255;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;0;False;;False;False;False;False;True;1;Lightmode=UniversalForward;True;7;False;0;Hidden/InternalErrorShader;0;0;Standard;24;Workflow;1;0;Surface;0;638654149579861417;Two Sided;1;0;Cast Shadows;1;0;  Use Shadow Threshold;0;0;GPU Instancing;0;0;Built-in Fog;1;0;Lightmaps;1;0;Volumetrics;1;0;Decals;0;0;Write Depth;0;0;  Early Z (broken);0;0;Vertex Position,InvertActionOnDeselection;1;0;Emission;1;0;PC Reflection Probe;3;0;PC Receive Shadows;1;0;PC Vertex Lights;0;0;PC SSAO;1;0;Q Reflection Probe;0;0;Q Receive Shadows;0;0;Q Vertex Lights;1;0;Q SSAO;0;0;Environment Reflections;1;0;Meta Pass;1;0;0;5;True;True;True;True;True;False;;False;0
...
WireConnection;321;1;105;0
WireConnection;321;0;331;0
WireConnection;322;1;110;0
WireConnection;322;0;331;0
WireConnection;323;1;109;0
WireConnection;323;0;331;0
WireConnection;324;1;106;0
WireConnection;324;0;331;0
WireConnection;325;1;107;0
WireConnection;325;0;331;0
WireConnection;330;1;324;0
WireConnection;330;0;106;0
WireConnection;326;1;323;0
WireConnection;326;0;109;0
WireConnection;327;1;322;0
WireConnection;327;0;110;0
WireConnection;328;1;321;0
...
WireConnection;312;3;265;0
WireConnection;312;4;145;0
WireConnection;312;6;144;0
WireConnection;312;7;146;0
WireConnection;312;8;317;0
ASEEND*/
//CHKSM=896A2542CA27C6E2E129AC7451112802673F89E9